...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  fn_copy2 = (char *)malloc(sizeof(char)*MAX_CMD_SIZE);
  strlcpy(fn_copy, file_name, MAX_CMD_SIZE);
  strlcpy(fn_copy2, file_name, MAX_CMD_SIZE);
              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
  ...
  while (read_bytes > 0 || zero_bytes > 0)
  {
    ...
    if (page_read_bytes == PGSIZE) { // all non-zeroed page
      ...
      /* Load this page. */
      if (file_read_at (file, kpage, page_read_bytes, ofs + PGSIZE*pos_index) != (int) page_read_bytes)
      {
